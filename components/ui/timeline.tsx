      ...
      <div className="max-w-7xl mx-auto pt-5 px-4 md:px-8 lg:px-10 sticky top-0 z-50 bg-white pb-14">
        ...
        <div className="flex gap-5 items-center">
          <div className="flex gap-2 items-center">
            ...
            <h2 className="">{tripData?.duration}</h2>
          </div>

          <div className="flex gap-2 items-center">
            <Wallet className="text-green-700" />
            <h2 className="">{tripData?.budget}</h2>
          </div>

          <div className="flex gap-2 items-center">
            <Users className="text-blue-700" />
            <h2 className="">{tripData?.group_size}</h2>
          </div>
        </div>
      </div>
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        {data.map((item, index) => (
          <div key={index} className="flex justify-start" >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-[28%]">
              <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-white dark:bg-black flex items-center justify-center">
                <div className="h-4 w-4 rounded-full bg-neutral-200 dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 p-2" />
              </div>
              <h3 className="hidden md:block text-xl md:pl-14 md:text-3xl font-bold text-neutral-600 dark:text-neutral-500 ">
                {item.title}
              </h3>
            </div>
